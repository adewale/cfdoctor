...
- [Workers limits](https://developers.cloudflare.com/workers/platform/limits/index.md) — Per-isolate memory limit and error outcomes, Worker startup time (1 s) and size limits, and the documented remedies (stream bodies, offload data, profile locally).
- [Workers errors and exceptions](https://developers.cloudflare.com/workers/observability/errors/index.md) — Validation errors including `Script startup exceeded memory limit` and `Script startup exceeded CPU time limit`, plus runtime error messages.
- [Profiling Memory](https://developers.cloudflare.com/workers/observability/dev-tools/memory-usage/index.md) — DevTools heap snapshots against a local `wrangler dev` session; the only documented memory profiler.
- [Durable Objects: Metrics and analytics](https://developers.cloudflare.com/durable-objects/observability/metrics-and-analytics/index.md) — Memory usage measured per isolate (shared by objects of one class), `durableObjectsPeriodicGroups` quantiles, and the leak-trend guidance.
- [Durable Objects: In-memory state](https://developers.cloudflare.com/durable-objects/reference/in-memory-state/index.md) — Instances may share global memory; observe in-memory state size on the memory chart.
