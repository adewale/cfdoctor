...
- Added Turnstile bot-verification coverage based on Cloudflare's Turnstile Spin skill (`cloudflare/skills` `turnstile-spin`, launched 2026-09-25 at https://blog.cloudflare.com/turnstile-spin/) and the current server-side validation, testing-key, and migration docs:
  - New evidence record `CFDOC-EVD-CF-TURNSTILE-SPIN` (official guidance) for the canonical contract. Tokens are forgeable, expire after 300 seconds, are single-use, and are at most 2048 characters. The backend must be the only Siteverify caller. The handler must reject malformed tokens, send `remoteip`, time out, fail closed, and require `success`, the expected `action`, and a deployment-specific hostname allowlist that never includes localhost in production. The ledger now has 32 records.
  - Scanner `0.5.0` adds six leads, each with a fixture: `CFDOC-SEC-TURNSTILE-NO-SITEVERIFY` (widget with no backend validation, which Spin's widget-recovery mode repairs), `CFDOC-SEC-TURNSTILE-CLIENT-SITEVERIFY` (browser-side Siteverify or a `VITE_`/`NEXT_PUBLIC_`-style secret), `CFDOC-SEC-TURNSTILE-UNCHECKED-RESULT` (no `success`, `action`, or `hostname` check; `response.ok` is not a pass), `CFDOC-SEC-TURNSTILE-VERIFY-HARDENING` (no timeout, 2048-character guard, or `remoteip`), `CFDOC-SEC-TURNSTILE-TEST-KEY` (always-pass dummy keys in production code, vars, or dotenv, including fallbacks when the real secret is unset), and `CFDOC-FIT-LEGACY-CAPTCHA` (reCAPTCHA/hCaptcha migration candidates, with Enterprise called out).
  - Markup, framework components, templates, and Python sources are now read for the Turnstile leads only; every other check keeps its file coverage. Test paths, saved/scraped third-party pages, and minified bundles are ignored after a portfolio run found scraped article attachments embedding other sites' widgets.
  - Cloudflare's documented dummy keys are no longer reported as committed credentials by `CFDOC-SEC-SECRET-ASSIGNMENT`; placement is judged by the new test-key lead instead.
  - Detection fixtures grew from 30 to 35: `turnstile-widget-no-siteverify`, `turnstile-siteverify-unchecked`, `turnstile-client-siteverify`, `legacy-captcha-migration`, and the zero-finding control `turnstile-canonical-safe`. The `secret-reference-not-literal` control now uses the canonical Spin verifier so it pins the new result/hardening leads quiet on the shape they recommend.
  - `config-and-security-checks.md` gained a Turnstile section: the contract, token lifecycle and widget reset, secrets and test keys, migration, where Turnstile fits (browser humans) versus rate limits or auth (APIs, CLI/MCP clients, webhooks, GET search), app-issued clearance cookies that amplify one solve, and Turnstile Analytics as account evidence. Two prompt-only matrix IDs cover the semantic parts: `CFDOC-SEC-TURNSTILE-UNPROTECTED-FORM` and `CFDOC-SEC-TURNSTILE-CLEARANCE-AMPLIFY`. Source maps, provenance, and link-policy anchors (server-side validation "Maximum length: 2048 characters", backend-only Siteverify, `timeout-or-duplicate`; testing "Always passes validation") were updated and verified live.
  - The skill description now names Turnstile. The trigger eval grew to 46 cases, adding a Turnstile audit positive, a "set up Turnstile" negative that belongs to `turnstile-spin`, and a physical-turnstile false friend.
  - `research/turnstile-spin-portfolio-review-2026-09-25.md` records the review of 30 recent `adewale/*` Cloudflare projects that motivated these checks.
