...
| CFDOC-COST-CONTAINER-IDLE-WINDOW | scanner-lead | COST | medium | [`cost-footguns.md`](cost-footguns.md) (Containers); [`product-fit-rubric.md`](product-fit-rubric.md) (Containers) | Container subclass declares no `sleepAfter` and does not manage the timer via `onActivityExpired()`/`renewActivityTimeout()`, so each wake bills provisioned memory/disk through the default idle window. Prose mentions of the API do not suppress the lead; a declaration or call does. |
| CFDOC-COST-CONTAINER-DUTY-CYCLE | skill-prompt-only | COST | — | [`product-fit-rubric.md`](product-fit-rubric.md) and [`cost-footguns.md`](cost-footguns.md) (Containers) | Whether Containers beat a VM/spot instance depends on awake-time duty cycle, not workload type; requires request-rate and awake-seconds evidence a regex cannot supply. Keep-warm pings are the specific shape that silently converts bursty billing to always-on. |
| CFDOC-REL-CONTAINER-DETACHED-DISPATCH | skill-prompt-only | REL | — | [`product-fit-rubric.md`](product-fit-rubric.md) (Containers); [`performance-and-reliability.md`](performance-and-reliability.md) (lifecycle) | A Worker returning before its container job finishes can have the job cut off unless the execution context is held open; also covers `SIGTERM` handling during rollouts and `max_instances` erroring rather than queueing. Requires reading dispatch/await semantics, not a pattern match. |
