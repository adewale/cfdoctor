...
21. Durable Object gotchas should be checked when DOs are present: duration/WebSockets/hibernation, close hygiene, `storage.list()` hot paths, unbounded `storage.sql.exec` SELECT scans (rows-read meter), DO-to-DO stub-call cycles and detached re-trigger chains, alarm recursion, sharding/hotspots, ephemeral object-per-idempotency-key patterns, backend-aware write coalescing/transactions, fan-out to many DOs, `ctx.waitUntil()` lifecycle, and KV-vs-DO-storage fit.
...
23. War-story scenario checks should be considered for matching products: recursive async work, DO-to-DO re-trigger loops, webhook abuse, static/media DDoS bandwidth, image variant explosions, uncached object hotlinks, idle paid resources, direct origin/bucket bypass, and spend-alerts-only controls.
...
29. Loop and runaway-cost findings should state expected detection latency under the account's current alerting: Cloudflare budget alerts are informational only (no pause/cap; $10 auto-created default on eligible Pay-as-you-go accounts), per-product notifications must match the meters actually in use, and daily billable-usage review or API polling is the reliable runaway-meter watch. Pair every loop fix with a detection recommendation, not just a code change.
30. Isolate memory should be checked as a shared, fixed budget: module-scope baseline (schema-library tool registries, eager validators, large Wasm) against the per-isolate limit and the deploy-time startup validation errors, request-path buffering (`request.arrayBuffer()`, upstream pass-through, unread `clone()`, node:fs temp files) against the same budget, and for Durable Objects the per-isolate measurement shared by every object of the class. Memory findings need a measurement path (`wrangler check startup`, dry-run bundle size, memory percentiles with deployment markers, `exceededMemory`/reset counts), not just a code pattern.
