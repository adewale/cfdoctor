...
- Completed the whatmedicaidpays.com D1 rows-read incident integration (scanner `0.3.7`), turning the accepted-but-unlinked `CFDOC-EVD-D1-134-BILL` ledger record (127,599,130,859 rows read, $134.14, ~765k-row database, https://fullstacksveltekit.com/blog/cloudflare-d1-bill) into runnable coverage:
  - War-story scenario #24: D1 bills rows *scanned*, so indexing is a billing control; unindexed `MAX`/`DISTINCT`/`WHERE` hot queries rescan the table per call, layout-level loaders multiply that by sitewide page views, and indexes underperform until `ANALYZE`/`PRAGMA optimize` populates `sqlite_stat1`.
  - Two scanner leads: `CFDOC-COST-D1-NO-INDEXES` (schema SQL creates tables while nothing in the project — SQL or ORM `index()`/`@@index` markers — creates a secondary index, and code runs filtered/aggregate queries; the fix text carries the ANALYZE/EXPLAIN QUERY PLAN step) and `CFDOC-COST-D1-LAYOUT-HOTPATH` (SvelteKit `+layout*`, Next.js `app/**/layout.*`, or Remix `app/root.*` queries D1 with no cache evidence in file).
  - Detection fixtures `d1-unindexed-hot-queries` (fires exactly the two new leads) and `d1-indexed-layout-cached` (remediated twin with composite indexes, ANALYZE note, and KV cache-aside; `max_findings: 0` precision control); `d1-nplus1-dashboard` now also requires the missing-index lead. The suite is `28/28` detection fixtures, with six new scanner unit tests.
  - Reference updates (`cost-footguns.md`, `performance-and-reliability.md`), coverage-matrix rows for the 62-check registry, ledger lineage (scenario 24, both checks, both fixtures), and a claim-ledger validator that now derives its scenario bound from `SCENARIO_MAX = 24`.
  - Verified the change with an 18-run matched Sonnet audit over the two new fixtures (current vs `origin/main` vs no skill, 3 runs each, leakage-stripped case copies): the `ANALYZE`/`sqlite_stat1` lesson appeared 3/3 with the current skill vs 0/6 without, incident-sourced basis 3/3 vs 0/6, and 0/9 false positives on the remediated control (`evals/results/sonnet-d1-fixture-2026-08-10.md`). The audits also caught two unintended fixture artifacts — route params bound to integer surrogate keys instead of joined through `hcpcs_codes`/`states`, and a missing Workers Static Assets block — both now fixed in the committed fixtures (detection eval still `28/28`, the bad fixture still emits exactly the two new leads, the control stays at zero findings).
