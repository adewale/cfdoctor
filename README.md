...
- **Memory and startup review** — separate module-scope baseline (schema-library tool registries, eager validators, large Wasm) from request data against the fixed per-isolate memory limit and the deploy-time startup validation, check request-path buffering, and for Durable Objects account for the isolate being shared by every object of the class.
...
Current proof from the latest validation run (2026-09-04; model-graded results pinned from 2026-07-11):

- Trigger eval: `46/46 = 100%` (`evals/results/latest.md`).
- Detection eval: `34/34` fixtures pass, including valid/malformed JSONC, Queue-DLQ controls, deeper DO sharding, per-consumer Queue matching, alarm guard precision, linked Stream preload, indirect self-fetch, full observability sampling, DO stub-call cycles with chain/guarded false-positive controls, unbounded DO SQL scans, module-scope schema weight with a lazy-schema control, and request/upstream body buffering with a streaming control (`evals/results/detection/latest.md`).
- Coverage matrix: consistent with the 64-check scanner registry.
- Evidence ledger: 33 structured records cover all 25 checklist scenarios and 34 detection fixtures with reciprocal lineage; newer records add direct D1 cost, Durable Object alarm, product-fit, three first-party Cloudflare outage postmortems, an explicit superseded-evidence disposition, the 2026-08 StandardAgents DO runaway-loop bill, Cloudflare budget-alert/billable-usage semantics, the 2026-08 Polylane Durable Object memory resets, and the documented isolate memory model.
...
- Skill Eval Harness: v0.6.0 strict leakage/ablation validation and manifest audit pass across 41 cases / 6 ablations (re-run 2026-09-04 with the two new memory cases; zero blockers).
...
- Current-source links: full network check re-run 2026-08-09 after adding the StandardAgents/billing sources — 487 URLs across 33 files, 0 dead; every newly added source resolved, and the 32 reported errors were sandbox-network artifacts (GitHub HTTP 403s through the proxy, archive.org connection resets) on previously verified URLs. The critical-anchor set grew to 13, and the new anchor phrases (budget alerts "informational only"/"do not pause", billable usage "monthly invoice", Durable Objects "rows read") were verified against the live pages. Generated/private eval paths and authenticated runtime API templates are excluded from current network health.
