...
| CFDOC-COST-D1-ISOLATE-CACHE | scanner-lead | COST | medium | [`cost-footguns.md`](cost-footguns.md) (D1 rows-read meter) and [`war-story-scenario-checklist.md`](war-story-scenario-checklist.md) §24; proposed in §"Checks to add or strengthen" | Expensive D1 aggregate cached only in per-isolate memory; the effective refresh rate scales with isolate churn, not the TTL. |
