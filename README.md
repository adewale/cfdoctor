...
- GPT-5.5 current-PR three-way eval: 24 cases / 72 outputs comparing PR head, current `origin/main`, and no skill. PR head scored 88.61% objective / 89.75% combined, versus main 93.12% / 94.31% and no skill 73.19% / 68.13%. The PR-versus-main objective difference was not significant (`p=0.184281`); PR-versus-no-skill lift was +15.42 points (`p=0.004882`). PR head used 34.6% more mean tokens and 15.2% more mean elapsed time than main (`evals/results/gpt-5.5-value/latest.md`).
