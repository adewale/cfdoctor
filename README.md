...
- GPT-5.5 expanded three-way eval: 28 cases / 84 outputs comparing PR head, current `origin/main`, and no skill. PR head scored 93.93% objective / 94.73% combined, versus main 89.18% / 90.40% and no skill 72.16% / 67.70%. The four new Wrangler snapshot cases scored 100% objective/combined for the PR versus 65.48%/66.96% for main. Legacy PR/main objective scores were effectively equal (92.92% vs 93.12%; paired `p=1.0`), while full PR-versus-no-skill lift was +21.77 points (`p=0.000244`) (`evals/results/gpt-5.5-value/latest.md`).
