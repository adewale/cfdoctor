...
| CFDOC-SEC-TLS-FLEXIBLE | scanner-lead | SEC | high | [`config-and-security-checks.md`](config-and-security-checks.md) (SSL/TLS mode) | Terraform sets SSL/TLS mode to Flexible or Off; both leave the origin leg unencrypted/cleartext. |
| CFDOC-SEC-TLS-FULL-NOT-STRICT | scanner-lead | SEC | medium | [`config-and-security-checks.md`](config-and-security-checks.md) (SSL/TLS mode) | Terraform sets SSL/TLS mode to Full (non-strict): encrypted but origin certificate unvalidated, so the origin leg is MITM-able. Only Full (strict) validates the origin cert. |
| CFDOC-CONFIG-PAGE-RULES-LEGACY | scanner-lead | CONFIG | low | [`config-and-security-checks.md`](config-and-security-checks.md) (Rules products) | Terraform manages `cloudflare_page_rule`; Page Rules entered maintenance mode 2025-01-06, migrate to Cache/Configuration/Origin/Redirect Rules. |
| CFDOC-CONFIG-DNSSEC-MISSING | scanner-lead | CONFIG | low | [`config-and-security-checks.md`](config-and-security-checks.md) (DNS/zone hardening) | Terraform manages a `cloudflare_zone` with no `cloudflare_zone_dnssec`; prompt to enable DNSSEC (also needs a registrar DS record, so it is a nudge, not a hard defect). |
| CFDOC-SEC-TURNSTILE-NO-SITEVERIFY | scanner-lead | SEC | medium | [`config-and-security-checks.md`](config-and-security-checks.md) (Turnstile server-side validation) | Turnstile client widget with no server-side `siteverify`/`cf-turnstile-response` read anywhere in repo source; verification may live in a separate service, hence low confidence. |
| CFDOC-COST-ASSETS-RUN-WORKER-FIRST | scanner-lead | COST | medium | [`cost-footguns.md`](cost-footguns.md) (Workers Static Assets billing) | `assets.run_worker_first` set to `true` or a broad glob routes free static-asset requests through the billable Worker. Static-assets analog of `CFDOC-COST-PAGES-FUNCTION-ROUTES`. |
| CFDOC-SEC-TUNNEL-CREDENTIALS | scanner-lead | SEC | high | [`config-and-security-checks.md`](config-and-security-checks.md) (Cloudflare Tunnel) | Committed cloudflared credentials JSON (AccountTag/TunnelID/TunnelSecret); a persistent inbound path to the origin, distinct from a generic leaked token. |
| CFDOC-SEC-TUNNEL-NO-ACCESS | scanner-lead | SEC | medium | [`config-and-security-checks.md`](config-and-security-checks.md) (Cloudflare Tunnel) | cloudflared/Terraform tunnel ingress hostname with no Cloudflare Access application/policy in the repo; a nudge to confirm intended public exposure (Access is optional per docs, and may live in the dashboard). |
