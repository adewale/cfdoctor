...
class ClaimLedgerTests(unittest.TestCase):
    ...
    def test_current_ledger_is_valid_and_superseded_source_is_resolved(self) -> None:
        # Validate at the ledger's own as_of date so a refresh does not require
        # editing this test; the mutation tests below keep a fixed reference date.
        as_of = dt.date.fromisoformat(self.ledger["as_of"])
        errors, warnings = module.validate(copy.deepcopy(self.ledger), as_of)
