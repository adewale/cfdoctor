...
```sh
python3 skill-eval-harness/skill_benchmark.py validate cfdoctor/evals/shared-benchmark.json
```

Prepare paired run tasks:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare cfdoctor/evals/shared-benchmark.json --split tune --out /tmp/cfdoctor-tasks.jsonl
```

Include ablation variants when running a focused regression check:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare cfdoctor/evals/shared-benchmark.json --split tune --include-ablations --out /tmp/cfdoctor-ablation-tasks.jsonl
```
...
```sh
python3 skill-eval-harness/skill_benchmark.py benchmark cfdoctor/evals/shared-benchmark.json --runs cfdoctor/eval-runs/latest --out /tmp/cfdoctor-benchmark.json
```
