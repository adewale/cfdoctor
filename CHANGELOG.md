...
- Added seven scanner checks (scanner `0.3.7`, registry now 67) grounded in current official Cloudflare docs, motivated by a review of the Orange Cloud Report (orangecloud.report) operator experience and verified against the docs:
  - `CFDOC-SEC-TLS-FULL-NOT-STRICT` — Terraform SSL/TLS mode Full (non-strict) encrypts the origin leg but does not validate the origin certificate (MITM-able); only Full (strict) validates. `CFDOC-SEC-TLS-FLEXIBLE` was strengthened to also grade `ssl = "off"` (no encryption) alongside Flexible. Source: `ssl/origin-configuration/ssl-modes/`.
  - `CFDOC-SEC-TURNSTILE-NO-SITEVERIFY` — a Turnstile client widget with no server-side `siteverify`/`cf-turnstile-response` verification anywhere in repo source (cosmetic, bypassable protection). Source: `turnstile/get-started/server-side-validation/`.
  - `CFDOC-COST-ASSETS-RUN-WORKER-FIRST` — `assets.run_worker_first: true`/broad glob routes free static-asset requests through the billable Worker (Static Assets analog of `CFDOC-COST-PAGES-FUNCTION-ROUTES`). Source: `workers/static-assets/billing-and-limitations/`.
  - `CFDOC-SEC-TUNNEL-CREDENTIALS` — a committed cloudflared credentials JSON (AccountTag/TunnelID/TunnelSecret), a persistent inbound path to the origin.
  - `CFDOC-SEC-TUNNEL-NO-ACCESS` — a cloudflared/Terraform tunnel ingress hostname with no Cloudflare Access policy in the repo (nudge; a tunnel public hostname is internet-reachable by default and Access is optional). Source: `cloudflare-one/connections/connect-networks/routing-to-tunnel/`.
  - `CFDOC-CONFIG-PAGE-RULES-LEGACY` — Terraform `cloudflare_page_rule`; Page Rules entered maintenance mode 2025-01-06, migrate to Cache/Configuration/Origin/Redirect Rules. Source: `rules/reference/page-rules-migration/`.
  - `CFDOC-CONFIG-DNSSEC-MISSING` — a Terraform-managed `cloudflare_zone` with no `cloudflare_zone_dnssec` (nudge; DNSSEC also needs a registrar DS record). Source: `dns/dnssec/`.
- Added seven detection fixtures (five positive, two negative/precision controls) and seven reciprocal evidence-ledger records; detection suite is now `33/33`, coverage matrix and claim ledger consistent, self-scan clean. The `turnstile-verified` control also asserts `max_findings: 0`, which holds now that `CFDOC-SEC-SECRET-ASSIGNMENT` no longer reports the `cf-turnstile-response` read it recommends.
- Reference/guidance updates grounded in current docs: Hyperdrive default-on query-caching read-after-write staleness; AI Gateway BYOK vs Unified Billing (~5% credit fee) trade-off; Browser Run 60s idle-timeout mid-work truncation and `keep_alive`; an Email Routing (inbound) vs Email Sending (outbound) product-fit section; Workflows hot-path anti-fit; a KV read-op billing clarification; Turnstile lockout/fallback UX; managed-WAF false positives on webhooks; graded TLS severities; plan-gated DNS record caps and WAF capability; and Cloudflare Tunnel as first-class origin-exposure remediation.
