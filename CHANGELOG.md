...
- Added Containers runtime coverage (scanner `0.3.7`). Containers were named in the skill description, detected as a binding, and had a pricing-source row, but had no product-fit, cost, or check coverage — they were inventoried and never diagnosed. Pipelines and Workers VPC remain in that state.
  - New `product-fit-rubric.md` and `cost-footguns.md` sections. Containers meter unlike anything else on the platform: memory and disk bill on the instance's *provisioned* size for every 10ms it is **awake** (only CPU tracks real use), charges stop at sleep rather than at request end, and — unlike Workers and R2 — Containers bill network egress. The "Cloudflare has no egress fees" intuition is wrong here.
  - New scanner lead `CFDOC-COST-CONTAINER-IDLE-WINDOW`: a Container subclass that never declares `sleepAfter` inherits the platform default (currently `"10m"`), so every wake carries an idle tail billed at full provisioned memory and disk. Suppressed by a `sleepAfter` declaration or an `onActivityExpired()`/`renewActivityTimeout()` call — a prose mention of the API does not suppress it. Added the `container-idle-window` fixture (27 detection fixtures).
  - New prompt-only rows `CFDOC-COST-CONTAINER-DUTY-CYCLE` (Containers price well for work that sleeps and badly for work that does not; workload type does not predict which) and `CFDOC-REL-CONTAINER-DETACHED-DISPATCH` (a Worker returning before its container job finishes can have the job cut off; also `SIGTERM` during rollouts and `max_instances` erroring rather than queueing).
- Added the 2026-07-26 compute landscape refresh (`research/compute-landscape-refresh-2026-07-26.md`) and expanded the evidence ledger from 29 to 32 records, retaining exactly 23 runtime checklist scenarios. Two independent first-hand production write-ups (VideoToBe, Kent C. Dodds) hit the container lifecycle problem from opposite directions — one had jobs killed mid-work, the other had to add explicit idle signalling "instead of relying on timeout windows." A third record captures an operator disagreement about Containers for identical media workloads, retained **unverified**, where the discriminator is duty cycle rather than workload type.
- `tests/test_claim_ledger.py` now validates the current ledger at its own `as_of` date instead of a hardcoded one, so a refresh no longer requires editing the test. The mutation tests keep a fixed reference date.
